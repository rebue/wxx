...
//    private final static String REFUND_URL               = "https://api.mch.weixin.qq.com/secapi/pay/refund";
    ...
    @Override
    public WxRefundRo wxRefund(final WxRefundTo to) throws SAXException {
        _log.info("微信退款：{}", to);
        final WxRefundRo ro = new WxRefundRo();
        _log.info("检验参数是否正确");
        if ((to.getOrderPayId() == null && to.getTransactionId() == null) || to.getOrderAmount() == null || to.getRefundAmount() == null || to.getRefundId() == null) {
            final String msg = "没有填写必要的参数: " + to;
            _log.warn(msg);
        ...
        // 签名
        SignUtils.sign2(requestParams, signKey);
        final String refundData = XmlUtils.mapToXml(requestParams);
        _log.info("请求退款的参数：" + refundData);
        String result = null;
        try {
            final WxPayConfigEx config = new WxPayConfigEx();
            ...
            final WXPayRequest wxPayRequest = new WXPayRequest(config);
        ...
        final WxRefundRo refundRo = new WxRefundRo();
